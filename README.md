...
This section covers calls and events that are used in connection with gathering data and consent at the same time.

## requestDataAndConsent(xid,refId,[campaign_id],[config])

Sends out the consent and data request to a specific user using the information in the campaign. The campaign definition determines what data we will ask from the user.

options:
```
xid           => XcooBee Id of the user from whom data is requested
refId         => optional: reference Id generated by you that identifies this request to you. Max 64 chars. This will returned to you in event response.
campaign_id   => optional: the campaign id to use if not default.
config        => optional: the config object
```

When user responds to the data request a webhook will fire from XcooBee to the identified endpoint in the campaign. The SDK does not allow Endpoints to be created or changed. Please use the GUI.
