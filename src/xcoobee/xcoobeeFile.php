...
class XcoobeeFile
{
    ...
    /**
     * Create a new xcoobeeFile instance
     *
    ...
    /**
     * Create a new xcoobeeFile instance using a file stream
     *
     * @param        $fileName
     * @param        $resource
     * @param string $mode
    ...
    {
        // create a new stream and set it to the xcoobee file
        $stream = \GuzzleHttp\Psr7\stream_for($resource);
        if (!$stream) {
            throw new ClientException('Failed to create File instance. Unable to open the given resource.');
        }
        ...
        $xcoobeeFile = new self($fileName, $mode);
        $xcoobeeFile->setStream($stream);

        return $xcoobeeFile;
    }

    /**
     * Create a new xcoobeeFile instance using a file path
     *
     * This behaves the same as the constructor but was added in order to
     * match the syntax of the static createByStream function
     *
     * @see xcoobeeFile::createByStream()
     *
     * @param $filePath
     * @param $mode
     *
    ...
    /**
     * Manually set the stream for this xcoobeeFile instance
     *
     * @param $stream
     */
    ...
    public function open()
    {
        ...
        if (!$this->isRemoteFile($this->path)) {
            ...
            if ($this->isNotReadable()) {
                throw new ClientException('Failed to create xcoobeeFile instance. Unable to read resource: ' . $this->path . '.');
            }

            // File is not Writable
            if ($this->isNotWritable()) {
                throw new ClientException('Failed to create File instance. Unable to write resource: ' . $this->path . '.');
            }
        }
        ...
        if (!$this->stream) {
            throw new ClientException('Failed to create File instance. Unable to open resource: ' . $this->path . '.');
        }
    }
